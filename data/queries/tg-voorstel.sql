WITH klacht AS
       (
         SELECT DISTINCT RIGHT(vav.objectid, 10) AS sehid,
                         vkl1.omschr             AS klacht
         FROM vrlijst_antwview vav
                LEFT OUTER JOIN vrlijst_vragen vvr
                                ON vav.realvrid = vvr.vraagid
                LEFT OUTER JOIN vrlijst_keuzelst vkl1
                                ON vvr.keuzelijst = vkl1.lijstcode AND SUBSTRING(vav.antwoord, 1, 10) = vkl1.code
...
SELECT TOP 100 mut.sehid      AS sehid,
               ...
               mut.bednr      AS bed,
               klacht.klacht  AS ingangsklacht,
...
FROM seh_sehmut mut
       ...
       LEFT OUTER JOIN klacht ON mut.sehid = klacht.sehid
